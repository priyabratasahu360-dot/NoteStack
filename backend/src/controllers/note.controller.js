...
export const getDownlodedNotes = async (req, res) => {
  ...
  try {
    const downloadedNotes = await DownloadedNote.find({
      userId: userId,
    }).populate({
      path: "noteId",
      populate: {
    ...
    const filteredNotes = downloadedNotes.filter(
...
export const getUploadedNotes = async (req, res) => {
  ...
  if (!userId) {
    return res
      .status(401)
      .json({ message: "You are Unauthorized - can't get uploaded notes" });
  }
  try {
    const uploadedNotes = await Note.find({ authorId: userId }).populate(
      "authorId",
      "_id email title description category tags keywords files",
    );
...
export const uploadNote = async (req, res) => {
  ...
  if (!title || !description) {
    return res
      .status(400)
      .json({ message: "Please add a title and description to your note" });
  }
  if (!req.file) {
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: "Note created succesfully", note: createdNote });
  } catch (error) {
...
export const deleteSelectedNote = async (req, res) => {
  ...
  try {
    const selectedNote = await Note.findOneAndDelete({
      _id: noteId,
      authorId: userId,
    });
...
export const getStats = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Application stats", totalUsers, totalDownloads });
  } catch (error) {
...
export const addLike = async (req, res) => {
  try {
    ...
    //converted objectId type to string
